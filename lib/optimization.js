...
Optimization.prototype.schedulecost = function (sol) {
	...
	for (var i = 0, j = sol.length / 2; i < j; i += 1) {
		...
		totalprice += outbound[2];  // [0]: 発時刻 [1]: 着時刻 [2]: 運賃
		totalprice += returnf[2];

		// 飛行時間1分0.5ドルとしてコストを追加
		totalprice += (this.getminutes(outbound[1]) - this.getminutes(outbound[0])) * 0.5;
		totalprice += (this.getminutes(returnf[1]) - this.getminutes(returnf[0])) * 0.5;

		// もし朝8時までに空港に行かなければいけないときは20ドル追加
		if (this.getminutes(outbound[0]) < this.getminutes('8:00')) {
			totalprice += 20;
		}
		if (this.getminutes(returnf[0]) < this.getminutes('8:00')) {
			totalprice += 20;
		}
...
// 模擬アニーリング
Optimization.prototype.annealingoptimize = function (domain, costf, T, cool, step, count) {
	T = T - 0 || 10000.0;
	cool = cool - 0 || 0.95;
	step = step - 0 || 1;
	count = count - 0 || 1;
	...
	// 模擬アニーリング(強化版)
	// 1個ではなくcount個の開始点から最適解を求める
	var best = 999999999, bestv;
	var T_org = T;
	...
	while (count--) {
		...
		// 現在のスコアを求める
		var current = costf.call(this, vec);
